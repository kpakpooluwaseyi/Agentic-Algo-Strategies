...
class VuManchuCipherBATR(MoonDevStrategy):
    # Optimizable parameters
    risk_pct = 1.0 # Not used directly for simple sizing but good for reference
    ema_period = 200
    atr_period = 14
    atr_stop_mult = 2.0
    atr_target_mult = 3.0
    ...
    def init(self):
        ...
        # Handle zero division? Likely rare in crypto volume but possible.
        # pandas handles / by 0 as inf.
    ...
    def next(self):
        ...
        if len(self.data) < max(self.ema_period, self.atr_period, 60):
            return
        ...
        overbought = self.wt2[-1] > 50 # Correction from < 60 to > 50
...
if __name__ == "__main__":
    ...
    if os.path.exists(data_path):
        ...
        # Run Backtest
        bt = Backtest(data, VuManchuCipherBATR, cash=1_000_000, commission=.002)
        stats = bt.run()
        print("\n--- BACKTEST RESULTS (CYCLE 2) ---")
        print(stats)
